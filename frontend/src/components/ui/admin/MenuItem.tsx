...
export default function MenuItem({
    ...
    return (
        <li>
            <button
                onClick={() => (hasSubMenu ? toggleDropdown(item.name) : navigate(item.href))}
                className={`flex items-center w-full px-3 py-2 space-x-3 rounded-xl text-sm font-semibold transition-all ${isActive
            ...
            >
                ...
                {isOpen && (
                    <div className="flex justify-between items-center w-full">
                        ...
                        {hasSubMenu && (
                            <BiChevronDown
                                className={`w-5 h-5 ml-auto transition-transform ${activeDropdown === item.name ? "rotate-180" : ""
                                    }`}
                            />
                        )}
                    </div>
