...
export default function AdminLayout() {
    ...
    return (
        <div className="flex min-h-screen bg-gray-100 text-black">
            {!isLoginPage && (
                <div
                    className={`h-full lg:block fixed z-50 mt-3 lg:top-0 transition-all duration-500 ease-in-out ${isSidebarOpen ? "w-64" : "w-0 lg:w-20"}`}
                    onMouseEnter={() => {
            ...
            <div className={`transition-all w-full duration-500 ease-in-out ${isSidebarOpen ? "lg:ml-64" : "lg:ml-20"}`}>
                {/* Navbar */}
                {!isLoginPage && (
                    <Navbar toggleSidebar={handleSidebarToggle} isSidebar={isSidebarOpen} />
                )}
                <div className={`pb-5 ${isSidebarOpen ? "px-4 lg:px-6" : "px-4 lg:px-20"} `}>
                    <Outlet />
                </div>
                {/* Footer */}
                {!isLoginPage && (
